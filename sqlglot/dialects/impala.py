...
def _str_to_date_sql(self: generator.Generator, expression: exp.StrToDate) -> str:
    ...
    if time_format not in (Impala.TIME_FORMAT, Impala.DATE_FORMAT):
...
def _time_format(
    ...
    time_format = self.format_time(expression)
    if time_format == Impala.TIME_FORMAT:
        return None
    return time_format
...
def _to_date_sql(self: generator.Generator, expression: exp.TsOrDsToDate) -> str:
    this = self.sql(expression, "this")
    time_format = self.format_time(expression)
    if time_format and time_format not in (Impala.TIME_FORMAT, Impala.DATE_FORMAT):
    ...
    class Parser(parser.Parser):
        ...
        FUNCTIONS = {
            **parser.Parser.FUNCTIONS,
            ...
            "MAP": parse_var_map,
            "MINUTE": lambda args: exp.Minute(this=exp.TsOrDsToDate(this=seq_get(args, 0))),
            "MINUTES_SUB": lambda args: exp.TsOrDsAdd(
                this=seq_get(args, 0),
                expression=exp.Mul(this=seq_get(args, 1), expression=exp.Literal.number(-1)),
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {
            **generator.Generator.TRANSFORMS,
            ...
            exp.DateToDi: lambda self, e: f"CAST(DATE_FORMAT({self.sql(e, 'this')}, {Impala.DATEINT_FORMAT}) AS INT)",
            exp.DiToDate: lambda self, e: f"TO_DATE(CAST({self.sql(e, 'this')} AS STRING), {Impala.DATEINT_FORMAT})",
